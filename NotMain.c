...
Pagetype SignUpPage(Node** Fams_Listpptr,Node** Users_Listpptr,int* active_userid_ptr,int* active_familyid_ptr)
{
    ...
    if(sc)
    {
        ...
        Node* temp = *Fams_Listpptr;
        Family* fptr =(Family*)(temp->data_ptr);
...
status_code CreateFamily(Node** Fams_Listpptr, Node** Users_Listpptr, int* active_userid_ptr, int* active_familyid_ptr, char* fam_name , char* user_name , float user_income )
{
    ...
    if(newFamData_ptr != NULL)
    {
        ...
        else
        {
            ...
            {
                Node* temp = *Fams_Listpptr;
                Family* data =(Family*) temp->data_ptr;
                printf("\nCHECK famname = %s",data->family_name);
            }
            ...
            if(sc != FAILURE)
            {
                ...
                if(newUserNode != NULL)
                {
                    ...
                    {
                        Family* famptr = (Family*)((*Fams_Listpptr)->data_ptr);
                        Node* user= famptr->family_members_ptr;
                        User* dataptr = (User*)user->data_ptr;
                        printf("\n\n\tUser name after adding to list = %s",dataptr->user_name);
                    }
                }
                else
                {
